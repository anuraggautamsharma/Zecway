...
export default function WaitlistForm({
  ...
  return (
    <div className={`w-full ${compact ? "max-w-md" : "max-w-md"}`}>
      <form
        onSubmit={submit}
        className={`flex gap-2 rounded-xl p-1.5 transition ${
          dark
            ? "border border-white/15 bg-dark-elevated focus-within:border-accent/60"
            : "border border-line bg-paper focus-within:border-ink/30"
        }`}
      >
        <input
          type="email"
          ...
          className={`min-w-0 flex-1 rounded-lg bg-transparent px-3.5 py-2.5 text-sm focus:outline-none ${
            dark
